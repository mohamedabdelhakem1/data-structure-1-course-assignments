...
public class PolynomialSolver extends JFrame {
	...
	public void splitter(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			if (Character.isDigit(s.charAt(i)) || s.charAt(i) == '-') {
				String ss = "";
				int k;
				for (k = i; Character.isDigit(s.charAt(k))
						|| (s.charAt(k) == '-'); k++) {
					ss += s.charAt(k);
				}
				i = k;
	...
	public void evaluate() {
		counter = 0;
		output.setText(
				"Insert the variable name: A, B , C , R and press enter");
		input.setText("");
		ActionListener ac2 = new ActionListener() {
			@Override
				...
				if (counter == 0) {
					...
					if (!s.isEmpty()) {
						...
						if (ch1 == 'A' || ch1 == 'B' || ch1 == 'C') {
							input.setText("");
							output.setText(
									"enter the value to be substituted in the form 8.00 (with the floating point) ");
							counter++;
						} else {
							output.setText("invalid input try again");
							input.setText("");
				...
				if (!s.isEmpty()) {
					...
					if (ch1 == 'A' || ch1 == 'B' || ch1 == 'C') {
						...
						if (poly.isEmpty(ch1)) {
							...
							output.setText(
									"the polynomial is not cleared , try again");
						}
					} else {
						output.setText("invalid input try again");
						input.setText("");
